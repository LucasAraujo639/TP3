...
def diametro(grafo):
    ...
    for v in grafo.obtener_vertices():
        camino_min_mas_grande = obtener_distancias_maximas_bfs(grafo,v)
...
def lectura(grafo, paginas):
    grados = grados_salida(grafo, paginas)
    vertices= vertices_entrada(grafo, paginas)
    cola = deque()
...
def comunidad(grafo, pagina):
    ...
    for _ in range(FACTOR_ITERACIONES_COMUNIDAD):
        ...
        for indice in range(0, cantidad_vertices):
            ...
            etiquetas[v] = max_frec(etiquetas, vertices_entrantes[v])
